...
import top.ibase4j.core.support.logger.Config;
import top.ibase4j.core.support.logger.Constant;
...
public class LogManager extends Thread {
    ...
    /** 日志写入的间隔时间 */
    public final static long WRITE_LOG_INV_TIME = Config.getInt("WRITE_LOG_INV_TIME", 1000);

    /** 单个日志文件的大小(默认为10M) */
    public final static long SINGLE_LOG_FILE_SIZE = 1024 * 1024 * Config.getInt("SINGLE_LOG_FILE_SIZE", 10);

    /** 缓存大小(默认10KB) */
    public final static long SINGLE_LOG_CACHE_SIZE = 1024 * Config.getInt("SINGLE_LOG_CACHE_SIZE", 10);
    ...
    public void addLog(String logFileName, StringBuffer logMsg) {
        ...
        synchronized (lfi) {
            ...
            lfi.currCacheSize += Config.StringToBytes(logMsg.toString()).length;
        }
    }
    ...
    public void run() {
        ...
        while (bIsRun) {
            try {
                ...
                // 重新获取日志级别
                if (i++ % 100 == 0) {
                    Constant.CFG_LOG_LEVEL = Config.getString("LOG_LEVEL", "0,1,2,3,4");
                    i = 1;
                }
            } catch (Exception e) {
    ...
    private void createLogFile(LogFileItem lfi) {
        // 当前系统日期
        String currPCDate = DateUtil.getDate();
        ...
        // 如果超过单个文件大小，则拆分文件
        if (lfi.fullLogFileName != null && lfi.fullLogFileName.length() > 0
    ...
    private int writeToFile(String sFullFileName, ArrayList<StringBuffer> sbLogMsg) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < sbLogMsg.size(); i++) {
                ...
                byte[] tmpBytes = Config.StringToBytes(logMsg.toString());
                fout.write(tmpBytes);
